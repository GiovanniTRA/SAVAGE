...
from SAVAGE.Utils.utils import build_data, initialize_adj_fn, initialize_P, initialize_P_fn, initialize_X_full, \
    import_initialized_P, initialize_X_full_ablation
...
def attack(cfg):
    ...
    X_full = initialize_X_full_ablation(data.x, cfg.max_fn, device, cfg.init_X)
    ...
    os.makedirs("../support_materials/results/ablations/X/", exist_ok=True)
    with open(f"../support_materials/results/ablations/X/"
              f"{cfg.data_name}_init_X_{cfg.init_X}_n_steps_{cfg.n_steps}.txt", "w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--init_X", default="random", type=str)
