...
from SAVAGE.Utils.utils import build_data, initialize_adj_fn, initialize_P, initialize_P_fn, initialize_X_full, \
    import_initialized_P, initialize_P_ablation
...
def attack(cfg):
    ...
    for exp in range(cfg.num_exp):
        ...
        if cfg.import_init:
            ...
            P_ori, P_nori = initialize_P_ablation(
                source, target, adj_fn, cfg.max_fn, device, init_P=cfg.init_P)
        ...
        with torch.no_grad():
            ...
            y_before_out = torch.sum(adj_fn, dim=1)
            y_before_in = torch.sum(adj_fn, dim=0)

            y_after_out = torch.sum(A_tilde_tr, dim=1)
            y_after_in = torch.sum(A_tilde_tr, dim=0)

            kl_in = kl_loss(F.log_softmax(y_after_in, dim=0), F.softmax(y_before_in, dim=0))
            kl_out = kl_loss(F.log_softmax(y_after_out, dim=0), F.softmax(y_before_out, dim=0))
            ...
            kl_in_mean = kl_in.item()
            kl_out_mean = kl_out.item()
    ...
    print(f"MEAN KL IN: {kl_in_mean / cfg.num_exp}")
    print(f"MEAN KL OUT: {kl_out_mean / cfg.num_exp}")
    print(f"ELAPSED TIME: {time.time() - start}")

    os.makedirs("../support_materials/results/ablations/init_P/", exist_ok=True)
    with open(f"../support_materials/results/ablations/init_P/"
              f"{cfg.data_name}_n_steps_{cfg.n_steps}_init_P_{cfg.init_P}.txt", "w") as f:
        ...
        f.write("\n")
        f.write(f"MEAN KL IN: {kl_in_mean / cfg.num_exp}")
        f.write("\n")
        f.write(f"MEAN KL OUT: {kl_out_mean / cfg.num_exp}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--init_P", default="random", type=str)
