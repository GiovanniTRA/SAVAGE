...
from SAVAGE.Utils.utils import build_data, initialize_adj_fn, initialize_P, initialize_P_fn, initialize_X_full, \
    import_initialized_P, initialize_P_fn_ablation
...
def attack(cfg):
    ...
    for exp in range(cfg.num_exp):
        ...
        P_fn = initialize_P_fn_ablation(cfg.max_fn, device, cfg.init_P_fn)
    ...
    os.makedirs("../support_materials/results/ablations/init_P_fn/", exist_ok=True)
    with open(f"../support_materials/results/ablations/init_P_fn/"
              f"{cfg.data_name}_n_steps_{cfg.n_steps}_init_P_fn_{cfg.init_P_fn}.txt", "w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--init_P_fn", default="random", type=str)
